      ...
      <form onSubmit={handleSignup}>
        <div>
          <Field icon={<UserRound size={17} />} label="FIRST NAME">
            <input type="text" placeholder="test" value={form.firstName} onChange={update('firstName')} required autoComplete="given-name" />
          </Field>
          <Field icon={<UserRound size={17} />} label="LAST NAME">
            <input type="text" placeholder="test" value={form.lastName} onChange={update('lastName')} required autoComplete="family-name" />
          </Field>
        </div>
        ...
        <Field icon={<Lock size={17} />} label="PASSWORD">
          <input type="password" placeholder="At least 8 characters" value={form.password}
            onChange={update('password')} minLength={8} required autoComplete="new-password" />
        </Field>

        {error && <ErrorBox message={error} />}

        <button disabled={loading} style={{ ...primaryButton }}>
          {loading ? <><Loader2 size={17} /> Creating account...</> : <>Create Account <ArrowRight size={17} /></>}
        </button>
